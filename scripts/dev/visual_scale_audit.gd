...
static func _samples() -> Array[Dictionary]:
	return [
		{"key": "BUILDING_WARREN", "path": _VisualCatalog.BUILDING_WARREN, "scale": _VisualCatalog.building_visual_scale(Defs.BuildingKind.WARREN), "min_m": 2.0, "max_m": 3.5, "axis": "xz"},
		{"key": "BUILDING_SLEEPING_PIT", "path": _VisualCatalog.BUILDING_SLEEPING_PIT, "scale": _VisualCatalog.building_visual_scale(Defs.BuildingKind.SLEEPING_PIT), "min_m": 1.6, "max_m": 3.0, "axis": "xz"},
		{"key": "BUILDING_WATCHTOWER", "path": _VisualCatalog.BUILDING_WATCHTOWER, "scale": _VisualCatalog.building_visual_scale(Defs.BuildingKind.WATCHTOWER), "min_m": 0.8, "max_m": 1.4, "axis": "xz"},
		{"key": "GOBLIN_WORKER", "path": _VisualCatalog.GOBLIN_WORKER, "scale": _VisualCatalog.unit_visual_scale(false, false, false), "min_m": 0.95, "max_m": 1.25, "axis": "y"},
		...
		{"key": "ENV_ROCK", "path": _VisualCatalog.ENV_ROCK, "scale": _VisualCatalog.env_visual_scale(_VisualCatalog.ENV_ROCK), "min_m": 1.0, "max_m": 2.0, "axis": "y"},
	]
...
static func _audit_sample(sample: Dictionary) -> String:
	...
	root.free()
	var measured := _measure_axis(aabb, axis)
...
static func _measure_axis(aabb: AABB, axis: String) -> float:
	match axis:
		"y":
			return aabb.size.y
		"x":
