class_name ShamanHut
extends Building

## Trains hobgoblin mages on a timer. Living mages generate passive magic.

const _COST := {
	Defs.ResourceKind.GOLD: Constants.SHAMAN_GOLD_COST,
	Defs.ResourceKind.FOOD: Constants.SHAMAN_FOOD_COST,
}
...
func _ready() -> void:
	super._ready()
	add_to_group(Defs.GROUP_SHAMAN_HUT)
	_ensure_progress_label("Mage")


func setup_shaman_hut(cell: Vector2i, def: BuildingDef, stockpile: Stockpile) -> void:
	setup(cell, def)
...
func tick_spawn(delta: float, colony: GoblinWarrenColony) -> void:
	...
	_update_progress_label(_spawn_timer, Constants.SHAMAN_TRAIN_INTERVAL, can_afford, can_house, "Mage")
	if _spawn_timer < Constants.SHAMAN_TRAIN_INTERVAL:
		return
	if not can_afford:
		return
	if not can_house:
		return
	if not colony.try_spawn_hobgoblin_mage_near(self):
		return
	_stockpile.spend(_COST)
	_spawn_timer = 0.0


## Called by the colony each frame: converts living hobgoblin mages into passive magic.
func tick_passive_magic(delta: float, tree: SceneTree) -> void:
	if _stockpile == null or tree == null:
		return
	var mages: Array = tree.get_nodes_in_group(Defs.GROUP_HOBGOBLIN_MAGE)
	if mages.is_empty():
		return
	var rate: float = float(mages.size()) * Constants.HOBGOBLIN_MAGE_PASSIVE_MAGIC_PER_SECOND
	_magic_accum += delta * rate
